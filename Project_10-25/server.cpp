...
int createSocket(int port)
{
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsd) != 0) {
		sprintf((char*)log_msg, "Winsock ³õÊ¼»¯Ê§°Ü!\n");
		log(log_msg);
		return 1;
	}
	...
	if (socket_fd<0)
	{
		sprintf((char*)log_msg, "Modbus Server: error creating stream socket => %s\n", strerror(errno));
		log(log_msg);
		return -1;
	}
	...
	{
		sprintf((char*)log_msg, "Modbus Server: error binding socket => %s\n", strerror(errno));
		log(log_msg);
		return -1;
	}
	...
	listen(socket_fd, 5);
	sprintf((char*)log_msg, "Modbus Server: Listening on port %d\n", port);
	log(log_msg);
...
int waitForClient(int socket_fd)
{
	...
	sprintf((char*)log_msg, "Modbus Server: waiting for new client...\n");
	log(log_msg);
...
int listenToClient(int client_fd, unsigned char *buffer)
{
	...
	int n = recv(client_fd, (char*)buffer, 1024, 0);
	return n;
}
...
void processMessage(unsigned char *buffer, int bufferSize, int client_fd)
{
	...
	recv(client_fd, (char*)buffer, messageSize, 0);
}
...
void *handleConnections(void *arguments)
{
	...
	sprintf((char*)log_msg, "Modbus Server: Thread created for client ID: %d\n", client_fd);
	log(log_msg);
	...
	while (run_modbus)
	{
		...
		if (messageSize <= 0 || messageSize > 1024)
		{
			...
			if (messageSize == 0)
			{
				sprintf((char*)log_msg, "Modbus Server: client ID: %d has closed the connection\n", client_fd);
				log(log_msg);
			}
			else
			{
				sprintf((char*)log_msg, "Modbus Server: Something is wrong with the  client ID: %d message Size : %i\n", client_fd, messageSize);
				log(log_msg);
			}
			break;
		}
...
void startServer(int port)
{
	...
	while (run_modbus)
	{
		...
		if (client_fd < 0)
		{
			sprintf((char*)log_msg, "Modbus Server: Error accepting client!\n");
			log(log_msg);
		}
		...
		else
		{
			...
			pthread_t thread;
			int ret = -1;
			sprintf((char*)log_msg, "Modbus Server: Client accepted! Creating thread for the new client ID: %d...\n", client_fd);
			log(log_msg);
	...
	closesocket(client_fd);
	WSACleanup();
	sprintf((char*)log_msg, "Terminating Modbus thread\r\n");
	log(log_msg);
}
