...
void querySlaveDevices()
{
	...
	for (int i = 0; i < num_devices; i++)
	{
		...
		{
			sprintf((char*)log_msg, "Device %s is disconnected. Attempting to reconnect...\n", mb_devices[i].dev_name);
			log(log_msg);
			if (modbus_connect(mb_devices[i].mb_ctx) == -1)
			{
				sprintf((char*)log_msg, "Connection failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
				log(log_msg);
			}
			else
			{
				sprintf((char*)log_msg, "Connected to MB device %s\n", mb_devices[i].dev_name);
				log(log_msg);
		...
		if (mb_devices[i].isConnected)
		{
			...
			if (mb_devices[i].input_registers.num_regs != 0)
			{
				...
				if (return_val == -1)
				{
					...
					sprintf((char*)log_msg, "Modbus Read Input Registers failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
					log(log_msg);
					bool_input_index += (mb_devices[i].discrete_inputs.num_regs);
				}
				else
			...
			if (mb_devices[i].holding_registers.num_regs != 0)
			{
				...
				if (return_val == -1)
				{
					...
					sprintf((char*)log_msg, "Modbus Write Holding Registers failed on MB device %s: %s\n", mb_devices[i].dev_name, modbus_strerror(errno));
					log(log_msg);
				}
